...
/**
 * alloc_read_nand_gpt_entries(): reads partition entries from disk
 * @offset - offset of nand gpt partition
 * @gpt - GPT header
 *
 ...
 */
static gpt_entry *alloc_read_nand_gpt_entries(unsigned long offset,
					 gpt_header *pgpt_head)
{
	size_t count = 0;
	gpt_entry *pte = NULL;
	...
	offset += (pgpt_head->partition_entry_lba*512);
	pte = (gpt_entry *)offset;
	return pte;
}


/**
 * is_nand_gpt_valid() - tests one GPT header and PTEs for validity
 *
 * offset - offset of nand gpt partition
 ...
 */
static int is_nand_gpt_valid(unsigned long offset,
			gpt_header *pgpt_head, gpt_entry **pgpt_pte)
{
	u32 crc32_backup = 0;
	u32 calc_crc32;
	...
	if (le64_to_cpu(pgpt_head->signature) != GPT_HEADER_SIGNATURE) {
		printf("GUID GPT Header signature is wrong 0x%llX != 0x%llX\n",
	...
	if (calc_crc32 != le32_to_cpu(crc32_backup)) {
		printf("GUID GPT Header CRC is wrong:0x%x != 0x%x\n",
		       le32_to_cpu(crc32_backup), calc_crc32);
		return 0;
	}

	/* Read and allocate Partition Table Entries */
	*pgpt_pte = alloc_read_nand_gpt_entries(offset, pgpt_head);
	if (*pgpt_pte == NULL) {
	...
	if (calc_crc32 != le32_to_cpu(pgpt_head->partition_entry_array_crc32)) {
		printf("GUID GPT Entry Array CRC is wrong:0x%x != 0x%x\n",
...
int add_mtd_parts_efi(gpt_header *gpt_head, gpt_entry *gpt_pte)
{
	int i;
	char ubootcmd[50];
	...
	if (run_command(ubootcmd, 0) == 0) {
		for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries);
		     i++) {
			partsize = (le64_to_cpu(gpt_pte[i].ending_lba) -
					le64_to_cpu(gpt_pte[i].starting_lba)
					+ 1)/2;
			printf("ending_lba =%#llx\n",
			       le64_to_cpu(gpt_pte[i].ending_lba));
			printf("starting_lba =%#llx\n",
			       le64_to_cpu(gpt_pte[i].starting_lba));
			sprintf(ubootcmd, "mtdparts add nand0 %dk %s",
				partsize, print_efiname(&gpt_pte[i]));
			printf("ubootcmd = %s\n", ubootcmd);
			if (run_command(ubootcmd, 0) == 0) {
				printf("mtd partition added syccessfully");
			} else {
				printf("ERROR:mtd partition added fail");
				return -1;
			}
		}
...
int print_nand_part_efi(unsigned long offset)
{
	gpt_header *gpt_head;
	gpt_entry *gpt_pte = NULL;
	int i = 0;
	...
	gpt_head = (gpt_header *)(offset+512);
	/* This function validates AND fills in the GPT header and PTE */
	if (is_nand_gpt_valid(offset, gpt_head, &gpt_pte) != 1) {
		printf("%s: *** ERROR: Invalid GPT ***\n", __func__);
		return -1;
	}
	...
	ret = add_mtd_parts_efi(gpt_head, gpt_pte);
	return ret;
}
#endif
