		       ...
		       struct mmc_data *data)
{
	...
	/* retry count has been increased to 50000 as the eMMC write command is
	 * taking longer time for sparse images flashing in eMMC*/
	unsigned int retry = 500000;
	...
	if (!(cmd->resp_type & MMC_RSP_PRESENT))
		...
		if (!(host->quirks & SDHCI_QUIRK_BROKEN_R1B)) {
			...
			/*
			 * Waiting for transfer complete interrupt
			 * for commands with busy may take longer
			 */
			retry = 200000;
		}
	} else
