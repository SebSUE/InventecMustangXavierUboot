...
bool gmac_check_tx_done(struct eth_dma *dma)
{
	...
	if (intstatus & (I_XI0 | I_XI1 | I_XI2 | I_XI3)) {
		xfrdone = true;
		/* clear the int bits */
		intstatus &= ~(I_XI0 | I_XI1 | I_XI2 | I_XI3 | 0x1000);
...
int gmac_check_rx_done(struct eth_dma *dma, uint8_t *buf)
{
	...
	debug("Rcv expect(0x%x) curr(0x%x) active(0x%x)\n", index, curr, active);
...
int gmac_mac_init(struct eth_device *dev)
{
	...
	tmp_env = getenv("esw_bypass");
	if (tmp_env && *tmp_env == '1') {
		printf("switch bypass mode\n");
		/* set switch bypass mode */
		eth->switchmode = false;
		tmp |= (1 << CDRU_SWITCH_BYPASS_SWITCH_SHIFT);
	} else {
		printf("default: switch mode\n");
		/* Swich mode */
		eth->switchmode = true;
		tmp &= ~(1 << CDRU_SWITCH_BYPASS_SWITCH_SHIFT);
		if (bcm_esw_init(eth))
	...
	/* Disable extra PAUSE frames */
	pause_ctrl = readl(UNIMAC0_PAUSE_CTRL_ADDR);
	pause_ctrl &= (uint)(~(1 << UNIMAC_PAUSE_CTRL_EN_SHIFT));
	writel(pause_ctrl, UNIMAC0_PAUSE_CTRL_ADDR);
...
int gmac_set_phy_speed(struct eth_device *dev, int speed)
{
	struct eth_info *eth = (struct eth_info *)(dev->priv);
	...
	if (eth) {
		...
		printf("%s: Failed to get eth device\n", __func__);
		return -1;
	}

	if (!phydev) {
		printf("%s: Failed to get phy device\n", __func__);
		return -1;
	}

	printf("eth port 0 speed = %d\n", speed);

	/* If speed is 100Mb, don't advertise Gb capability */
	if (speed == 100) {
		/* Disable 1000Base-T advertisement */
		features = phydev->supported;
		features &= ~(SUPPORTED_1000baseT_Full |
					  SUPPORTED_1000baseT_Half);
		phydev->advertising = features;

		/* Re-configure auto negotiation */
...
int gmac_add(struct eth_device *dev)
{
	 ...
	 * But DMA engine only uses lower 13-bit for addressing
	 * so that have to make it 16KiB (13-bit) aligned
	 */
	tmp = malloc(sizeof(dma64dd_t) * TX_BUF_NUM + 0x2000);
	if (tmp == NULL) {
	...
	dma->tx_desc_aligned = (void *)((((uint32_t)tmp) + 0x2000) & (~0x1fff));
	 ...
	 * so that have to make it 16KiB (13-bit) aligned
	 */
	tmp = malloc(sizeof(dma64dd_t) * RX_BUF_NUM + 0x2000);
	if (tmp == NULL) {
	...
	dma->rx_desc = tmp;
	dma->rx_desc_aligned = (void *)((((uint32_t)tmp) + 0x2000) & (~0x1fff));
...
#ifdef CONFIG_BCM_ETH_0_100MB
	eth->set_phy_speed = gmac_set_phy_speed;
#endif
