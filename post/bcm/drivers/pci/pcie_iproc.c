...
#define PAXB_1_CONFIG_IND_ADDR 0x18013120
#define PAXB_1_CONFIG_IND_DATA 0x18013124
#define PAXB_0_CLK_CONTROL 0x18012000
#define PAXB_0_CLK_CONTROL__RC_PCIE_RST_OUTPUT 0
#define PAXB_1_CLK_CONTROL__RC_PCIE_RST_OUTPUT 0
...
#define PCI_CLASS_BRIDGE_PCI_MASK  0xFF0000FF
        pci_bus0_read_config_dword(pcie_port, 0, PCI_BRIDGE_CTRL_REG_OFFSET, &val);
        val = (val & PCI_CLASS_BRIDGE_PCI_MASK) | (PCI_CLASS_BRIDGE_PCI << 8);
        pci_bus0_write_config_dword(pcie_port, 0, PCI_BRIDGE_CTRL_REG_OFFSET, val);
...
#define PCI_TARGET_LINK_SPEED_GEN1    0x1
                pci_bus0_read_config_dword(pcie_port, 0,
                ...
                if ((val & PCI_TARGET_LINK_SPEED_MASK) ==
                        ...
                        pci_bus0_write_config_dword(pcie_port, 0,
                                        PCI_LINK_STATUS_CTRL_2_OFFSET, val);
                        pci_bus0_read_config_dword(pcie_port, 0,
                                        PCI_LINK_STATUS_CTRL_2_OFFSET, &val);
                        mdelay(100);

                        pos = pci_bus0_find_capability(pcie_port, 0, PCI_CAP_ID_EXP);
                        pci_bus0_read_config_word(pcie_port, 0, pos + PCI_EXP_LNKSTA,
                                        &tmp16);
...
int  pcie0_wr_ind_reg(uint32_t addr, uint32_t data)
{
    ...
    if (conf_trace) printf("PCIE indirect reg write: Addr = 0x%08X Data = 0x%08X\n", addr, data);
    return 0;
}
...
int  pcie0_rd_ind_reg(uint32_t addr, uint32_t *data)
{
    reg32_write(PAXB_0_CONFIG_IND_ADDR, addr);
    *data = reg32_read(PAXB_0_CONFIG_IND_DATA);
    if (conf_trace) printf("PCIE indirect reg read:  Addr = 0x%08X Data = 0x%08X\n", addr, *data);
    return 0;
}
...
static inline int mdio_wait_idle(void)
{
        int timeout = MDIO_TIMEOUT_USEC;

        while (readl(ChipcommonB_MII_Management_Control + MII_MGMT_CTRL_OFFSET) &
...
static void mdio_init(void)
{
        ...
        writel(val, ChipcommonB_MII_Management_Control + MII_MGMT_CTRL_OFFSET);

        if (mdio_wait_idle())
		printf("%s() - Error\n", __func__);
}

static u16 mdio_read(unsigned int phy_addr, unsigned int reg_addr)
{
        u32 val;
	...
	val= phy_read(phy_addr, reg_addr);
        ...
        writel(val, ChipcommonB_MII_Management_Control + MII_MGMT_CMD_DATA_OFFSET);
...
static u16 iproc_pcie_phy_reg_read(unsigned int phy_addr, unsigned int reg_addr)
{
        u16 val;

        mdio_write(phy_addr, PCIE_PHY_BLK_ADDR_OFFSET,
                        reg_addr & PCIE_PHY_BLK_ADDR_MASK);
        val = mdio_read(phy_addr, reg_addr & PCIE_PHY_REG_ADDR_MASK);

	if (conf_trace)
		printf("%s() phy rd: phy: 0x%0x reg: 0x%4x data: 0x%4x\n",
			__func__, phy_addr, reg_addr, val);

        return val;
}

static void iproc_pcie_phy_reg_write(unsigned int phy_addr, unsigned int reg_addr, u16 val)
{
        mdio_write(phy_addr, PCIE_PHY_BLK_ADDR_OFFSET,
                        reg_addr & PCIE_PHY_BLK_ADDR_MASK);
        mdio_write(phy_addr, reg_addr & PCIE_PHY_REG_ADDR_MASK, val);

	if (conf_trace)
		printf("%s() phy wr: phy: 0x%0x reg: 0x%4x data: 0x%4x\n",
			__func__, phy_addr, reg_addr, val);
}

#define PCIE_PHY_REG_ADDR 0x2103
#define PCIE_PHY_DATA     0x2B1C
static void iproc_pcie_mii_phy_init(u32 phy_addr)
{
...
static void soc_pcie_hw_pre(int port)
{
    ...
    if (conf_trace) printf("PAXB_0_CLK_CONTROL = 0x%08X\n", data);
    ...
    for(i=0; i<2; i++) {
        if (conf_trace)  printf("Addr = 0x%08X Data = 0x%08X\n", (u32)addr, *addr);
        addr++;
    }

    for(i=0; i<2; i++) {
        pcie0_rd_ind_reg(i*4, &data);
        if (conf_trace)  printf("Local PCIE Header Reg = 0x%08X Data = 0x%08X\n", i*4, data);
    }
   
    //----------------------------------------------------

    phy_addr = 5;
	iproc_pcie_mii_phy_init(phy_addr);
    //----------------------------------------------------
    } else {
       data = reg32_read(PAXB_1_CLK_CONTROL);
    if (conf_trace)  printf("PAXB_1_CLK_CONTROL = 0x%08X\n", data);
    //data &= ~(1 << PAXB_0_CLK_CONTROL__EP_PERST_SOURCE_SELECT);
...
int iproc_pcie_rd_conf_dword(struct pci_controller *hose, pci_dev_t dev, int offset, u32 * value)
{
	...
	if ((PCI_FUNC(dev) > 1) || (PCI_DEV(dev) > 0))
...
static void soc_pcie_bridge_init(struct soc_pcie_port * port)
{
        ...
        if (conf_trace) printf("membase %#x memlimit %#x\n",
...
//		bus_num = 0;
	...
	if (bus_alloc[port] == 0) {
		...
		hose->first_busno = port;  //bus_num;
		hose->last_busno = port; //bus_num;
		hose->current_busno = port; //bus_num++;
