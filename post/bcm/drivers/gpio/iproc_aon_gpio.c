...
void iproc_aon_gpio_irq_handler(void *data)
{
	...
	for (iter = 0; iter < numberOfAonGPIOs; iter++) {
		if (tmp & (1 << iter)) {
			...
			/* clear the interrupt*/
			int_mask = readl(GP_INT_CLR);
			writel(int_mask | (1 << iter), GP_INT_CLR);
...
void iprocAonGPIOInit(void)
{
	...
	gpioDev.ioaddr = (void *)(GP_DATA_IN-0x800);
	gpioDev.dmu_ioaddr = 0;/*(void*)NORTHSTAR_DMU_BASE;*/
	gpioDev.intr_ioaddr = (void *)GP_DATA_IN;/*CCA_CHIPID;*/
	gpioDev.irq_base = 0;
}
