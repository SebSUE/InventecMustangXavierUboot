...
#if defined(CONFIG_CYGNUS) && defined(CONFIG_LCD)
#if CONFIG_POST & CONFIG_SYS_POST_LCD
#include "../../drivers/video/cygnus_asiu_lcd.h"
#include "../../include/cygnus_pl111.h"
...
{
	uint32_t save = readl(a);/*Save initial val*/
	uint32_t status = TEST_PASS;

	post_log("address = 0x%X\n", a);
	post_log("value0 = 0x%X\n", readl(a));

	writel(REG_SET_VAL1, a);
	post_log("value1 = 0x%X\n", readl(a));
	if (REG_SET_VAL1 != readl(a))
		status = TEST_FAIL;

	writel(REG_SET_VAL2, a);
	post_log("value2 = 0x%X\n", readl(a));
	if (REG_SET_VAL2 != readl(a))
		status = TEST_FAIL;

	writel(REG_SET_VAL3, a);
	post_log("value3 = 0x%X\n", readl(a));
	if (REG_SET_VAL3 != readl(a))
		status = TEST_FAIL;

	writel(save, a);/*restore saved val*/
	return status;
}
...
{
	uint32_t d = readl(a);/*Save initial val*/
	uint32_t status = TEST_PASS;
	post_log("address = 0x%X\n", a);
	post_log("value = 0x%X\n", d);
	writel((~d), a);
...
{
	int status = TEST_PASS;
	struct clcd_registers *r = (struct clcd_registers *)LCD_BASE_ADDRESS;

	/* r/w register test */
	if (TEST_PASS !=	read_write_reg_test((uint32_t)&r->tim0)) {
		post_log("error : Register TIM0 failed read-write test\n");
		status = TEST_FAIL;
	}
	if (TEST_PASS !=	read_write_reg_test((uint32_t)&r->tim1)) {
...
int lcd_palette_RAM_test(void)
{
	...
	for (loop_index = 0; loop_index < l; loop_index += 1) {
		...
		v = readl(&r->palette[loop_index]);
		/*post_log("V0 = 0x%X\n",v);*/
		writel((~v), &r->palette[loop_index]);
		check = (~v) & 0xFFFFFFFF;
		rv1 = readl(&r->palette[loop_index]);
		/*post_log("V1 = 0x%X\n",rv1);*/
		if (check != rv1) {
		...
		writel((~rv1), &r->palette[loop_index]);
		check = (~rv1) & 0xFFFFFFFF;
		rv2 = readl(&r->palette[loop_index]);
		/*post_log("V2 = 0x%X\n",rv2);*/
		if (check != rv2) {
...
int lcd_cur_image_RAM_test(void)
{
	...
	for (loop_index = 0; loop_index < l; loop_index++) {
		/*post_log("A = 0x%X\n",&r->palette[loop_index]);*/
		v = readl(&r->curimg[loop_index]);
		/*post_log("V0 = 0x%X\n",v);*/
		writel((~v), &r->curimg[loop_index]);
		rv1 = readl(&r->curimg[loop_index]);
		/*post_log("V1 = 0x%X\n",rv1);*/
		if (v != ~(rv1)) {
		...
		writel((~rv1), &r->curimg[loop_index]);
		rv2 = readl(&r->curimg[loop_index]);
		/*post_log("V2 = 0x%X\n",rv2);*/
		if (v != rv2) {
...
#if 0
	int status = TEST_PASS;
	uint32_t reg_data = readl(ASIU_TOP_LCD_AXI_SB_CTRL);
