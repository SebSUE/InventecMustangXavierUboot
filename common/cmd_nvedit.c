...
struct env_print_one_ctx {
	int flags;
	int len;
};

static int env_print_one(void *context, ENTRY *ep)
{
	struct env_print_one_ctx *cx = context;

	if ((cx->flags & H_HIDE_DOT) != 0 && ep->key[0] == '.')
		return 0;

	if ((cx->flags & H_PRNT_READABLY) == 0) {
		cx->len += printf("%s=%s\n", ep->key, ep->data);
		return 0;
	} else {
		char *s;

		cx->len += printf("setenv %s '", ep->key);
...
static int env_print(char *name, int flag)
{
	...
	if (name) {		/* print a single name */
		...
		e.key = name;
		e.data = NULL;
		hsearch_r(e, FIND, &ep, &env_htab, cx.flags);
		if (ep == NULL)
			return 0;
		env_print_one(&cx, ep);
		return cx.len;
	...
	if (hwalk_ctx_r(&env_htab, &cx, env_print_one)) {
		/* should never happen */
		printf("## Error: cannot print environment\n");
		return 0;
	}
			...
			char * const argv[])
{
	...
	while (argc > 1 && argv[1][0] == '-') {
		switch (argv[1][1]) {
		case 'a': /* All, including leading dot vars */
			argc--;
			argv++;
			env_flag &= ~H_HIDE_DOT;
			continue;
		case 'r': /* Print machine-readably as setenv commands */
			argc--;
			argv++;
			env_flag |= H_PRNT_READABLY;
			continue;
		default:
			printf("## %s: option \"%s\" not defined\n",
			       cmdtp->name, argv[1]);
			return 1;
		}
...
U_BOOT_CMD_COMPLETE(
	...
	"[-a]\n"
	"    - print all [include hidden] environment name=value pairs.\n"
	"      (Hidden variables are those named with a leading dot).\n"
	"printenv -r [-a]\n"
	"    - print all vars [include hidden], in a machine-readable way.\n"
	"      (Formatted as properly escaped 'setenv' command syntax).\n"
	"printenv [-r] name ...\n"
	"    - print [machine-readably] the environment variables specified\n"
