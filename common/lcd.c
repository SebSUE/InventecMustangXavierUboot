...
static int lcd_init(void *lcdbase)
{
	...
	/*
	 * keep the lcd_is_enabled flag off to prevent
	 * writing stdout to the lcd screen.
	 */
	lcd_is_enabled = 1;
#endif
	lcd_clear();
	...
	if (bpix < 12) {
		...
		for (i = 0; i < ARRAY_SIZE(bmp_logo_palette); ++i) {
			...
			*(cmap + BMP_LOGO_OFFSET) = lut_entry;
			cmap++;
#elif defined(CONFIG_PL111_LCD) && defined(CONFIG_CYGNUS)
			lcd_setcolreg(i, ((colreg & 0x0F00) >> 4),
					colreg & 0x00F0,
					(colreg & 0x000F) << 4);
...
int lcd_display_bitmap(ulong bmp_image, int x, int y)
{
	...
	if (bmp_bpix == 8) {
		...
		for (i = 0; i < colors; ++i) {
			...
			*cmap = 0xffff - colreg;
#else
#if defined(CONFIG_CYGNUS) && defined(CONFIG_PL111_LCD)
